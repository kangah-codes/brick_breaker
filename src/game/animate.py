...
class Animation():
    ...
    def __init__(self, sprites, scale, frame_duration=0.1):
        ...
        """
        self.images = [pygame.transform.scale(image, (int(image.get_rect().width * scale), int(image.get_rect().height * scale))) for image in sprites]
    ...
    def __str__(self):
        return f"<Animation with sprites {self.images}>"
